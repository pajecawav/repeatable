...
function StreakEntry({ streak, best }: { streak: Streak; best: number }) {
	...
	const paddingElement =
		percentageOfBest < 1 ? (
			<div style={{ flexGrow: (1 - percentageOfBest) / 2 }} />
		) : null;
...
export function HabitBestStreaksChart({ habit }: HabitBestStreaksChartProps) {
	const streaks = useStreaks(habit);

	const bestStreak = useMemo(
		() =>
			streaks.reduce((best, streak) => Math.max(best, streak.length), 0),
		[streaks]
	);
